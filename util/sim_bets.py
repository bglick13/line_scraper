from util.line_scraper_functions import abbrev_to_name, nba_mapping, nfl_mapping
import numpy as np
import pandas as pd
import pymc3 as pm
import matplotlib.pyplot as plt
from util.line_scraper_functions import process_games
...
if __name__ == '__main__':
    ...
    lines = pd.read_pickle('../nba_c_lines500.pkl')
    ...
    for z in z_tols:
        outcomes, probs, br = simulate_games(lines, nba_games, nba_mapping, tol=z, risk=.05, use_z=True)
        fig, ax = plt.subplots(figsize=(17, 8))
        ax.plot(range(len(br)), br)
        plt.savefig('z{}_returns.png'.format(z))
    ...
    for p in p_tols:
        outcomes, probs, br = simulate_games(lines, nba_games, nba_mapping, tol=p, risk=.05, use_z=False)
        fig, ax = plt.subplots(figsize=(17, 8))
        ax.plot(range(len(br)), br)
        plt.savefig('p{}_returns.png'.format(p))
        avg_return = np.mean(outcomes)
        percent_bet = float(len(outcomes)) / float(len(lines.game_uuid.unique()))
        accuracy = np.mean(np.array(outcomes) > 0)
        with pm.Model() as model:
